    ...
    with tf.Session(config=tf_config) as sess:
        ...
        for i in range(config['runs']):
            ...
            if params.get('is_accompaniment'):
                ...
                if(i<1):
                    feed_dict_sampler[placeholder_c]=feed_dict_sampler[placeholder_c][0]
                    feed_dict_sampler[placeholder_c] =np.expand_dims(feed_dict_sampler[placeholder_c],0)
                    tmp = feed_dict_sampler[placeholder_c]
                    np.save('input_data',tmp)
                    print(i,tmp.shape)
